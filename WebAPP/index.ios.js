...
// 引入自定义视图
import BathroomPage from './ios_views/BathroomPage';
...
export default class WebAPP extends Component {
  ...
  render() {
    return (
      // jsx 语法 unselectedTintColor 文字颜色是黑色，tintColor TabBarIOS 切换颜色是蓝色
      <TabBarIOS unselectedTintColor="black" tintColor="#0c92dc">
        <TabBarIOS.Item
          title="bathroom"
          // uri 图片路径，scale 缩放比例 3.5
          icon={{uri: img1_base64, scale: 3.5}}
          // 针对选择状态，选择显示视图
          selected={this.state.selectedTab === 'Bathroom'}
          onPress={() => {
            this.setState({
              selectedTab: 'Bathroom',
            });
          }}>
        {this._renderView('BathroomPage')}
        </TabBarIOS.Item>
        <TabBarIOS.Item
          title="Read"
 ...
 _renderView(name){
   ...
   let view = null;
    switch (name){
      case 'BathroomPage':
        view = <BathroomPage ></BathroomPage>;
        break;
      case 'Read':
        view = <Read></Read>;
        break;
